...
def estimate_temperature(raw_temp_f: float) -> float:
    """
    Temperature, estimated using a formula temp = 1.0227 * raw_temp_f - 9.3755
    developed by Lance Wallace to account for heat generated by the WiFi module
    and other electronics in a PurpleAir sensor. This will not be accurate for
    all situations.

    :param raw_temp_f: Raw temperature in Fahrenheit from sensor
    """
    try:
        temp_float = float(raw_temp_f)
    except (ValueError, TypeError):
        raise ValueError(f"Temperature value ({raw_temp_f}) is not a number")
    corrected = 1.0227 * temp_float - 9.3755
    return round(corrected, 2)


def estimate_humidity(raw_relative_humidity: float) -> float:
    """
    Relative humidity, estimated using a formula
    relative_humidity = 1.4498 * raw_relative_humidity + 7.022
    developed by Lance Wallace to account for heat generated by the WiFi module
    and other electronics in a PurpleAir sensor. This will not be accurate for
    all situations.

    :param raw_relative_humidity: Raw relative humidity from sensor
    """
    try:
        rh_float = float(raw_relative_humidity)
    except (ValueError, TypeError):
        raise ValueError(f"Relative Humidity value ({raw_relative_humidity}) is not a number")
    corrected = 1.4498 * rh_float + 7.022
    return round(corrected, 2)